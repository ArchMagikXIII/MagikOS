-- Window and layer rules for the Magikos Quickshell surfaces. The
-- shell-wide bar / menu / popouts are layer-shell.

-- Keep the bar instant: no layer-shell fade/slide animation.
hl.layer_rule({ match = { namespace = "magikos-bar" }, no_anim = true, animation = "none" })
...
hl.layer_rule({ match = { namespace = "^(magikos-menu|magikos-image-selector|magikos-emojis|magikos-clipboard|magikos-keyboard-panel)$" }, no_anim = true, animation = "none" })

-- Dev gallery is the main shell workbench; open it maximized like
-- SUPER+ALT+F so component previews have the whole workspace.
o.window({ class = "^org.quickshell$", title = "^Magikos shell – dev gallery$" }, { maximize = true })
