...
Read this when the user wants to report an Magikos bug, suggest a feature, or
contribute a fix upstream.

Magikos lives at https://github.com/basecamp/magikos. Route requests to the
right place:

- **Verified bugs** -> GitHub issues. Issues are for validated bugs only, not
  support requests.
- **Feature ideas and suggestions** ->
  https://github.com/basecamp/magikos/discussions/categories/suggestions
- **Support and "is this a bug?" questions** -> the Discord community at
  https://magikos.org/discord. Start here when the problem isn't clearly a bug
  in Magikos itself.

## Filing a Good Bug Report

The bug template asks for system details (CPU, GPU, Magikos version), a
description with steps to reproduce, and diagnostics. Gather them:

```bash
magikos version

# Generate the diagnostic log (also written to /tmp/magikos-debug.log)
magikos debug --no-sudo --print

# Interactive variant: `magikos debug` offers to upload the log to
# logs.magikos.org (expires after 24h) and prints a shareable URL to
...
`magikos capture screenshot` and `magikos screenrecord`. Keep recordings short
...
`MAGIKOS_SCREENRECORD_DEBUG=true` and attach `/tmp/magikos-screenrecord.log`.
...
Never develop against `/usr/share/magikos`. Clone a working copy instead:

```bash
gh repo fork basecamp/magikos --clone
cd magikos
```
