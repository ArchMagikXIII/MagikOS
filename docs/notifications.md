...
`~/.local/state/magikos/notifications/` (one JSON line per file, named
`<timestamp>-<id>.json`), so live toasts survive the shell restart that
`magikos-update` performs. When a toast leaves the screen — expiry, dismissal,
...
`~/.local/state/magikos/notifications.json` and toggled via shell IPC
(`magikos-shell notifications toggleDnd` / `setDnd` / `dndState`).
`magikos-toggle-notification-silencing` wraps the toggle and refreshes the
bar's `magikos.indicators` widget, whose Dnd indicator binds directly to the
service's `doNotDisturb` property.

Two kinds of notification punch through DND, chosen to be intentional and
rare:

- `app_name` = `magikos-action` — Magikos's own user-action confirmation
...
`notify-send`/`magikos-action`) are dropped entirely.

## The sender contract

`bin/magikos-notification-send` is the one way Magikos code sends
...
|---|---|---|
| `-g` / `--glyph` | `--hint=string:magikos-glyph:` | Nerd Font glyph for the icon slot when no image icon resolves |
| `--exec` | `--hint=string:magikos-exec:` | shell command the card runs when clicked |
| `--image` | `--hint=string:image-path:` | the standard freedesktop image hint |
| `--app-name` | `-a` | defaults to `magikos-action` |
| `-u` / `--urgency` | `-u` | defaults to `low` |

The defaults are the point: an unadorned `magikos-notification-send "Done"`
...
- `magikos-notification-wait [timeout]` — polls until the shell answers IPC
...
- `magikos-notification-dismiss <summary>` — dismiss by summary substring,
  used by the first-run toasts once their action has been clicked.
- `magikos-notification-time` / `-battery` — the hotkey notices: one-line
  low-urgency glyph toasts wrapping `date` and `magikos-battery-status`.
...
- **Low battery** — `magikos-battery-low` sends a critical toast and runs the
  `battery-low` hook.
- **Crash capture** — `magikos-crash-watch` follows the systemd-coredump
  journal stream and announces each crashed program (deduped per minute) as a
  critical toast whose `--exec` runs `magikos-agent-crash`. It waits for the
  server first: a shell crash takes the notification server down with it, and
  that crash is the one most worth reporting.
- **Pending migrations** — `magikos-migrate-notify` (from its user service
  after `graphical-session.target`) waits for the server, then sends a
  critical toast whose click opens a terminal running `magikos-migrate`,
...
`bin/magikos-reminder <minutes> [message]` creates a transient systemd user
timer via `systemd-run --user --collect --on-active=<minutes>m` under the
unit name `magikos-reminder-<minutes>m-<epoch>`; the timer's payload sends the
reminder toast, deletes its message file, and refreshes the bar indicator.
Custom messages are stashed in `$XDG_RUNTIME_DIR/magikos-reminders/<unit>.message`
...
`systemctl --user list-timers "magikos-reminder-*.timer"` — `show` as a
summary toast, `show --json` as the JSON the bar's Reminder indicator polls
(refreshed by the same `magikos-shell -q magikos.indicators refresh` call the
timers and mutations make). `magikos-reminder -i` summons the
`magikos.reminders` overlay (`shell/plugins/reminders/ReminderFlow.qml`), a
two-step minutes/message prompt that shells back out to `magikos-reminder` to
do the setting.
