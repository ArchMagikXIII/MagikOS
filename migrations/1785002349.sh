...
current_absolute_target="$HOME/.local/state/magikos/current/theme/neovim.lua"
...
# one ("../../../../.config/magikos/current/theme/neovim.lua"), leaving those
# links dangling. Match the shared suffix instead so every spelling is repaired.
case "$target" in
  */magikos/current/theme/neovim.lua) ;;
  *) exit 0 ;;
esac
