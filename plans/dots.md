...
rev 3 names the feature **dots** (`magikos dots ...`) and adds the
multi-machine sync design.

## Problem

Magikos declares `~/.config` "your files" but does little to preserve them:

- `magikos-refresh-config` litters `*.bak.<timestamp>` files next to originals.
- `magikos-reinstall-configs` clobbers everything back to `/etc/skel` defaults.
...
- **Raw git passthrough / lazygit over `$HOME`** (rejected in review):
  ...
  raw invocation themselves; Magikos will not bless it.
...
```bash
git init --bare ~/.local/share/magikos/dots.git   # mode 0700
```
...
- Repo-local config only: synthetic identity (`Magikos <magikos@localhost>`),
...
- Never export `GIT_DIR`/`GIT_WORK_TREE` (would leak into `magikos-plugin-*`,
  `magikos-theme-update`, `magikos-update-dev`, user hooks).
...
- **Best-effort everywhere**: a failed snapshot warns and continues. History
  ...
  `magikos-update`/`magikos-migrate` (`set -e` there must not see git errors).

### Audited manifest, not a derived whitelist

Tracking is `git add -f --pathspec-from-file=<manifest>` only. The manifest is
a hand-audited file shipped with Magikos — explicitly **not** derived from
...
- Exclude (deliberately): `.config/magikos/plugins/**` and
  `.config/magikos/themes/**` (nested git clones managed by
  `magikos-plugin-*` / `magikos-theme-update`), backgrounds (multi-MB
...
- At seed time and before each snapshot: if manifest paths are symlinks or a
  ...
  detected, mark the repo dormant and say so in `magikos dots status`.
...
internal `magikos-dots-snapshot "<label>"`:

1. **`magikos-provision-user`**: seed repo + initial commit, after `git.sh`
   and `.XCompose` are in place. Idempotent; `--force` must not re-init.
2. **`magikos-migrate`**: before/after pair labeled with version. Covers both
...
5. **Timer**: an hourly-ish systemd user timer committing only if dirty, so
   ...
   fails for any edit not followed by an Magikos operation.
...
- **`magikos dots push`** — *squash-publishes* the current manifest state
...
- **`magikos dots pull [url]`** — fetches `sync`, takes a local pre-pull
...
one `magikos dots restore` away.

Remote setup UX:

- `magikos dots push` with no remote runs setup inline: paste a git URL, or
...
- Dormant mode (dotfile-manager users) disables sync too — they have their
  own.
- v2 candidate: `magikos-provision-first-run` asks "Got a dots repo?" so a
...
invoked as `magikos dots <verb>`:

- `magikos dots snapshot [label]` — take a snapshot now
- `magikos dots log` — history with labels
- `magikos dots diff [ref]` — default range: the last before/after pair
- `magikos dots restore <file> [--at <ref>]` — accepts `~/.config/...`,
...
- `magikos dots push` / `magikos dots pull [url]` — sync (above)
- `magikos dots status` — repo state, dormant/active, remote, last
  snapshot/publish
...
- New users: seeded by `magikos-provision-user`.
...
- Docs/tests alignment: manual `31-dotfiles.md` gets the built-in story
  ...
  per the documentation layout; `default/agents/skills/magikos/SKILL.md`
...
1. Manifest location: `default/magikos/dots-manifest` vs alongside the
   helper; and whether users may extend it via
   `~/.config/magikos/dots-manifest.d/`.
2. Whether `.config/magikos/hooks/**` belongs in the manifest at all (small
   scripts, but the likeliest place for a pasted token).
3. Harden `magikos-refresh-config` against `..` path escape (documented today
