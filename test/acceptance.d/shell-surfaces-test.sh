...
open_and_close() {
  ...
  if [[ -n $payload ]]; then
    magikos-shell shell summon "$plugin" "$payload" >/dev/null
  else
...
magikos-shell shell summon magikos.emojis >/dev/null
wait_until "emoji picker opens" 15 layer_present "magikos-emojis"
wtype "rocket"
sleep 1
screenshot "success-emoji-picker-search"
wtype -k Return
wait_until "emoji picker selection closes" 15 layer_absent "magikos-emojis"

# Seed two clipboard entries, search for the older one, and copy it back out.
clipboard_token="Magikos acceptance clipboard $(date +%s)"
printf '%s' "$clipboard_token" | wl-copy
wait_until "clipboard history captures test text" 15 grep -Fq "$clipboard_token" "$HOME/.local/state/magikos/clipboard-history.json"
printf '%s' "clipboard decoy" | wl-copy
sleep 1

magikos-shell shell summon magikos.clipboard >/dev/null
wait_until "clipboard opens" 15 layer_present "magikos-clipboard"
wtype "$clipboard_token"
wait_until "clipboard search finds test text" 15 screen_contains "Magikos acceptance clipboard"
screenshot "success-clipboard-search"
wtype -M shift -k Return -m shift
wait_until "clipboard selection closes" 15 layer_absent "magikos-clipboard"
...
magikos-shell shell summon magikos.menu '{"menu":"system"}' >/dev/null
wait_until "system menu opens" 15 layer_present "magikos-menu"
wait_until "system menu content is visible" 15 screen_contains "Shutdown"
screenshot "success-system-menu"
wtype -k Escape
wait_until "system menu closes" 15 layer_absent "magikos-menu"
...
launch_app "magikos-theme-bg-switcher"
wait_until "background selector opens" 30 layer_present "magikos-image-selector"
sleep 1
screenshot "success-background-selector"
wtype -k Escape
wait_until "background selector closes" 15 layer_absent "magikos-image-selector"

launch_app "magikos-theme-switcher"
wait_until "theme selector opens" 30 layer_present "magikos-image-selector"
sleep 1
screenshot "success-theme-selector"
wtype -k Escape
wait_until "theme selector closes" 15 layer_absent "magikos-image-selector"

# Walk the reminder flow through each input screen, but dismiss before it
# schedules a real timer in the test user's session.
magikos-shell shell summon magikos.reminders >/dev/null
wait_until "reminder flow opens" 15 layer_present "magikos-reminders"
...
wtype -k Escape
wait_until "reminder flow closes" 15 layer_absent "magikos-reminders"

# Render a real shell notification and clear it through the notification IPC.
magikos-shell notifications dismissAll >/dev/null
magikos-notification-send "Acceptance notification" "Shell notification rendering" --expire-time=15000
wait_until "notification popup opens" 15 layer_present "magikos-notifications"
...
magikos-shell notifications dismissAll >/dev/null
wait_until "notification popup closes" 15 layer_absent "magikos-notifications"
...
magikos-menu summon apps >/dev/null
wait_until "apps menu opens" 15 layer_present "magikos-menu"
sleep 1
...
wait_until "apps menu closes after launching" 15 layer_absent "magikos-menu"
