...
MAGIKOS_TEST_NOTIFY_ARGS="$args_file" PATH="$tmpdir:$ROOT/bin:$PATH" \
  magikos-notification-send --app-name custom-app -g K -u critical --image /tmp/image.png \
  --exec "magikos-menu-keybindings 'a b'" "Learn Keybindings" "Body"
...
[[ ${args[4]} == "--hint=string:magikos-glyph:K" ]] || fail "notification wrapper converts glyph to hint"
[[ ${args[5]} == "--hint=string:image-path:/tmp/image.png" ]] || fail "notification wrapper converts image to hint"
[[ ${args[6]} == "--hint=string:magikos-exec:magikos-menu-keybindings 'a b'" ]] || fail "notification wrapper converts exec to hint"
...
: >"$args_file"
MAGIKOS_TEST_NOTIFY_ARGS="$args_file" PATH="$tmpdir:$ROOT/bin:$PATH" \
  magikos-notification-send "Plain" >/dev/null

grep -q "magikos-exec" "$args_file" && fail "notification wrapper adds no exec hint without --exec"
pass "notification wrapper omits the exec hint when no command is given"

if MAGIKOS_TEST_NOTIFY_ARGS="$args_file" PATH="$tmpdir:$ROOT/bin:$PATH" \
  magikos-notification-send "Headline" --exec 2>/dev/null; then
