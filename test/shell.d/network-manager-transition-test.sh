...
! grep -F 'systemd-networkd' "$dns" >/dev/null || fail "magikos-dns no longer restarts systemd-networkd"
grep -F 'NetworkManager/conf.d/20-magikos-dns.conf' "$dns" >/dev/null
...
if grep -F 'nmcli general reload conf,dns-full' "$dns" >/dev/null; then
  fail "magikos-dns must not push DNS before reapplying active profiles"
fi
pass "magikos-dns configures DNS through NetworkManager"
...
grep -F 'magikos-networkd-retired' "$hardware_network" >/dev/null
pass "hardware setup retires archinstall networkd state"

grep -F 'MAGIKOS_UPGRADE_TO_QUATTRO_LIVE' "$migration" >/dev/null
