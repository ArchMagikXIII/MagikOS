...
cat >"$stub_bin/magikos-shell" <<'SH'
#!/bin/bash

printf '%s\n' "$*" >>"$SHELL_CALLS"
SH

chmod +x "$stub_bin/magikos-update-available" "$stub_bin/magikos-shell"

PATH="$stub_bin:$PATH" SHELL_CALLS="$shell_calls" UPDATE_AVAILABLE_STATUS=0 \
  "$ROOT/bin/magikos-update-status"
grep -Fx -- "-q magikos.system-update refresh" "$shell_calls" >/dev/null ||
...
PATH="$stub_bin:$PATH" SHELL_CALLS="$shell_calls" UPDATE_AVAILABLE_STATUS=1 \
  "$ROOT/bin/magikos-update-status"
grep -Fx -- "-q magikos.system-update clear" "$shell_calls" >/dev/null ||
