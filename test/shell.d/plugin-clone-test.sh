...
mkdir -p "$TMPDIR/home/.config/magikos" "$TMPDIR/bin"
CALLS="$TMPDIR/calls"
...
if [[ $* == *"listShellConfig"* ]]; then
  ...
  if [[ ${FAKE_NO_DISCOVERY:-0} == 1 ]]; then
    ...
    find "$HOME/.config/magikos/plugins" -mindepth 2 -maxdepth 2 -name manifest.json -print0 |
      xargs -0 -r jq -s 'map({id: .id, enabled: true})'
  fi
elif [[ $* == *"setPluginEnabled"* ]]; then
  printf 'magikos-shell %s\n' "$*" >>"$FAKE_CALLS"
  printf 'ok\n'
fi
exit 0
SH

for command in magikos-plugin-enable magikos-notification-send fake-editor; do
...
clone_plugin() {
  local default_config='{
    "bar": {
      "layout": {
        "left": [{"id": "magikos.menu"}],
        "center": [{"id": "magikos.clock", "format": "HH:mm"}],
        "right": []
      }
    }
  }'
  HOME="$TMPDIR/home" USER=tester MAGIKOS_PATH="$ROOT" PATH="$TMPDIR/bin:$ROOT/bin:$PATH" \
    FAKE_CALLS="$CALLS" MAGIKOS_TEST_ROOT="$ROOT" \
    FAKE_SHELL_CONFIG="${FAKE_CLONE_CONFIG:-$default_config}" \
    magikos-plugin-clone "$@"
}

clone_plugin magikos.clock >/dev/null
clock="$TMPDIR/home/.config/magikos/plugins/tester.clock"
...
rg -qF "magikos.clock" "$clock" -g '*.qml' -g '*.js' ||
...
jq -e '
  ...
  .magikos.clonedFrom == "magikos.clock" and
...
grep -qx 'magikos-plugin-enable tester.clock' "$CALLS" ||
  fail "clone does not enable the editable copy"
grep -qx 'magikos-notification-send -g 󰐱 Editing Cloned Plugin Original plugin has been replace by clone.' "$CALLS" ||
...
clone_plugin magikos.keyboard-layout >/dev/null
grep -qx 'magikos-plugin-enable tester.keyboard-layout' "$CALLS" ||
...
clone_plugin magikos.menu >/dev/null
menu="$TMPDIR/home/.config/magikos/plugins/tester.menu"
...
grep -qx 'magikos-plugin-enable tester.menu' "$CALLS" ||
  fail "clone does not enable a multi-kind plugin"
pass "clone preserves and enables multi-kind plugins"

remove_output=$(HOME="$TMPDIR/home" MAGIKOS_PATH="$ROOT" PATH="$TMPDIR/bin:$ROOT/bin:$PATH" \
  FAKE_CALLS="$CALLS" MAGIKOS_TEST_ROOT="$ROOT" \
  magikos-plugin-remove tester.menu --yes)
grep -qx 'magikos-shell shell setPluginEnabled tester.menu false' "$CALLS" ||
  fail "removing an enabled clone does not disable it first"
grep -q 'Restored magikos.menu.' <<<"$remove_output" ||
...
clone_plugin magikos.active-window >/dev/null
[[ -f $TMPDIR/home/.config/magikos/plugins/tester.active-window/ActiveWindow.qml ]] ||
  fail "flat bar plugin clone is incomplete"
pass "flat bar plugins clone from adjacent manifests"

grep -qx 'magikos-plugin-enable tester.active-window' "$CALLS" ||
  fail "clone does not activate a bar widget whose source is absent"
pass "clone activates an absent bar widget"

clone_plugin magikos.indicators >/dev/null
indicators="$TMPDIR/home/.config/magikos/plugins/tester.indicators"
...
clone_plugin magikos.tray >/dev/null
[[ -f $TMPDIR/home/.config/magikos/plugins/tester.tray/TrayModel.js ]] ||
  fail "tray clone is missing its model"
pass "flat bar plugins keep local script dependencies"

clone_plugin magikos.bar >/dev/null
grep -qx 'magikos-plugin-enable tester.bar' "$CALLS" ||
  fail "clone does not select a cloned bar"
pass "clone switches full bars"

clone_plugin magikos.background >/dev/null
grep -qx 'magikos-plugin-enable tester.background' "$CALLS" ||
  fail "clone does not enable an ordinary cloned plugin"
pass "clone switches ordinary plugins"

EDITOR=fake-editor clone_plugin magikos.weather --edit >/dev/null
grep -qx "fake-editor $TMPDIR/home/.config/magikos/plugins/tester.weather" "$CALLS" ||
  fail "clone --edit does not open the clone in EDITOR"
pass "clone --edit opens the clone in EDITOR"
rm -rf "$TMPDIR/home/.config/magikos/plugins/tester.weather"

mkdir -p "$TMPDIR/home/.config/magikos/plugins/acme.example"
cat >"$TMPDIR/home/.config/magikos/plugins/acme.example/manifest.json" <<'JSON'
...
if clone_plugin magikos.weather custom.weather >/dev/null 2>&1; then
  fail "clone accepts a custom id"
fi
[[ ! -e $TMPDIR/home/.config/magikos/plugins/tester.weather ]] ||
  fail "rejected custom id leaves a clone behind"
pass "clone derives the personal id from the username"

if clone_plugin magikos.weather --replace >/dev/null 2>&1; then
...
if FAKE_NO_DISCOVERY=1 clone_plugin magikos.osd >/dev/null 2>&1; then
  fail "clone succeeds before the shell discovers it"
fi
[[ ! -e $TMPDIR/home/.config/magikos/plugins/tester.osd ]] ||
