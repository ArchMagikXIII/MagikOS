...
# skip belonged. Probe the socket, then Sway itself, since a compositor that
...
compositor_reachable() {
  ...
  # A compositor that died can leave its socket behind, so ask Sway whether it
  # is still answering. Only when it can be asked: swaymsg needs SWAYSOCK (or
  # I3SOCK), and treating a missing socket path as a dead compositor would skip
  # tests that would have run fine.
  if [[ -n ${SWAYSOCK:-} ]]; then
    swaymsg -t get_version >/dev/null 2>&1 && return 0
    return 1
  fi
  [[ -n ${I3SOCK:-} ]] && swaymsg -t get_version >/dev/null 2>&1 && return 0
