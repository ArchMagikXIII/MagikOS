...
other_packages="$ROOT/install/magikos-other.packages"
...
cat >"$stub_bin/magikos-pkg-present" <<'SH'
#!/bin/bash

(( ${TINY_DFR_INSTALLED:-0} == 1 ))
SH

cat >"$stub_bin/magikos-pkg-drop" <<'SH'
#!/bin/bash

printf 'magikos-pkg-drop' >>"$TEST_LOG"
printf '\t%s' "$@" >>"$TEST_LOG"
printf '\n' >>"$TEST_LOG"
SH
...
cat >"$limine_conf" <<'EOF'
# Generated by Magikos installer for T2 Mac support
...
PATH="$stub_bin:$PATH" \
  ...
  MAGIKOS_T2_LIMINE_CONF="$limine_conf" \
  MAGIKOS_T2_FAN_CONF="$fan_conf" \
  MAGIKOS_T2_RUNNING_CMDLINE="$running_cmdline" \
  MAGIKOS_T2_REPAIR_MARKER="$repair_marker" \
...
grep -Fq $'magikos-pkg-drop\ttiny-dfr' "$calls" ||
...
! grep -Eq $'^(sudo\t)?(sed|tee|systemctl|magikos-pkg-drop)(\t|$)' "$calls" ||
