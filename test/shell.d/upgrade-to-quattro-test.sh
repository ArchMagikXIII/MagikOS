...
upgrade_to_quattro="$ROOT/bin/magikos-upgrade-to-quattro"
...
grep -F 'magikos-snapshot create || (($? == 127))' "$upgrade_to_quattro" >/dev/null
pass "Magikos 4 upgrade snapshots the system before mutation"
...
grep -F 'pacman -Syy --noconfirm archlinux-keyring magikos-keyring' "$upgrade_to_quattro" >/dev/null
if grep -F 'pacman -Sy --noconfirm archlinux-keyring magikos-keyring' "$upgrade_to_quattro" >/dev/null; then
  fail "Magikos 4 upgrade forces a database refresh before installing keyrings"
fi
pass "Magikos 4 upgrade forces a database refresh before installing keyrings"

grep -F 'pacman -Syu --needed' "$upgrade_to_quattro" >/dev/null
grep -F 'magikos-update-aur-pkgs' "$upgrade_to_quattro" >/dev/null
grep -F 'magikos-update-available' "$upgrade_to_quattro" >/dev/null
grep -F 'magikos-update-mise' "$upgrade_to_quattro" >/dev/null
grep -F 'run_final_system_package_upgrade' "$upgrade_to_quattro" >/dev/null
pass "Magikos 4 upgrade completes package update checks"

grep -F 'run_post_upgrade_migrations' "$upgrade_to_quattro" >/dev/null
grep -F 'magikos-migrate' "$upgrade_to_quattro" >/dev/null
grep -F 'dust' "$upgrade_to_quattro" >/dev/null
grep -F 'satty' "$upgrade_to_quattro" >/dev/null
pass "Magikos 4 upgrade applies packaged migrations"

if grep -F 'skip-first-run-update-notification' "$upgrade_to_quattro" >/dev/null; then
  fail "Magikos 4 upgrade does not use notification-specific first-run state"
fi
pass "Magikos 4 upgrade completes first-run as one lifecycle"
...
pass "Magikos 4 upgrade completes first-run and migrates legacy completion markers"
...
if grep -F '"$root/bin/magikos-done"' "$upgrade_to_quattro" >/dev/null; then
  fail "Magikos 4 upgrade writes completion markers without the packaged magikos-done"
fi
pass "Magikos 4 upgrade writes completion markers without the packaged magikos-done"

for guarded_step in magikos-refresh-applications 'magikos-bar defaults'; do
  grep -F "run_as_user_magikos $guarded_step ||" "$upgrade_to_quattro" >/dev/null ||
    fail "Magikos 4 upgrade survives a packaged tree without $guarded_step"
done
pass "Magikos 4 upgrade survives a packaged tree missing top-level commands"

grep -F 'configure_snapper_policy' "$upgrade_to_quattro" >/dev/null
grep -F '/usr/share/magikos/install/config/snapper.sh' "$upgrade_to_quattro" >/dev/null
grep -F 'bash -euo pipefail "$snapper_config_script"' "$upgrade_to_quattro" >/dev/null
pass "Magikos 4 upgrade normalizes Snapper retention"

grep -F 'configure_lock_authentication' "$upgrade_to_quattro" >/dev/null
grep -F 'MAGIKOS_INSTALL_USER="$target_user"' "$upgrade_to_quattro" >/dev/null
grep -F '"$apply_lock"' "$upgrade_to_quattro" >/dev/null
pass "Magikos 4 upgrade configures lock screen authentication for the target user"

grep -F 'MAGIKOS_UPGRADE_TO_QUATTRO_LIVE=1' "$upgrade_to_quattro" >/dev/null
...
pass "Magikos 4 upgrade retires systemd-networkd for NetworkManager"
...
if function_body cleanup_retired_services | grep -F 'systemctl disable iwd' >/dev/null; then
  fail "Magikos 4 upgrade does not retire iwd in a step separate from the NetworkManager enable"
fi
grep -A1 -F '  enable_system_service NetworkManager.service' "$upgrade_to_quattro" |
  ...
  fail "Magikos 4 upgrade retires iwd in the step that enables NetworkManager"
pass "Magikos 4 upgrade switches from iwd to NetworkManager atomically"
...
grep -Fx 'trap cleanup_on_exit EXIT' "$upgrade_to_quattro" >/dev/null ||
  fail "Magikos 4 upgrade reports an aborted run instead of exiting silently"
...
grep -F 'exit "$exit_status"' <<<"$cleanup_body" >/dev/null ||
  fail "Magikos 4 upgrade preserves the failing exit status"
grep -F '>&2' <<<"$cleanup_body" >/dev/null ||
  fail "Magikos 4 upgrade reports an aborted run on stderr"
...
pass "Magikos 4 upgrade reports an aborted run instead of exiting silently"
...
! grep -q 'start_magikos_shell_session' "$upgrade_to_quattro" ||
  fail "Magikos 4 upgrade does not start the shell in the session it is replacing"
! grep -q 'stop_retired_session_processes' "$upgrade_to_quattro" ||
  fail "Magikos 4 upgrade leaves the retired session processes running until reboot"
pass "Magikos 4 upgrade leaves the Magikos 3 session alone until the reboot"

grep -F 'magikos-bar defaults' "$upgrade_to_quattro" >/dev/null
pass "Magikos 4 upgrade restores service-aware bar defaults"
...
pass "Magikos 4 upgrade backfills hardware support from the legacy release"

grep -F 'magikos-refresh-applications' "$upgrade_to_quattro" >/dev/null
pass "Magikos 4 upgrade refreshes application launchers"

grep -F '/etc/systemd/system.conf.d/99-magikos-nofile.conf' "$upgrade_to_quattro" >/dev/null
grep -F '/etc/systemd/user.conf.d/99-magikos-nofile.conf' "$upgrade_to_quattro" >/dev/null
pass "Magikos 4 upgrade removes stale nofile drop-ins"

cmdline_line=$(grep -n '^preserve_kernel_cmdline_root$' "$upgrade_to_quattro" | cut -d: -f1)
packages_line=$(grep -n '^install_magikos_quattro_packages$' "$upgrade_to_quattro" | cut -d: -f1)
...
pass "Magikos 4 upgrade preserves the kernel cmdline root parameters"
...
pass "Magikos 4 upgrade asks limine-entry-tool whether root= survives"
...
pass "Magikos 4 upgrade repair path refuses a partial dm-crypt cmdline"
...
grep -F "as_root find /boot/EFI/Linux -maxdepth 1 -name 'magikos_linux*.efi'" "$upgrade_to_quattro" >/dev/null
...
pass "Magikos 4 upgrade verifies the UKIs and refuses to reboot unverified"
