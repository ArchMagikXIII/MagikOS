...
cat >"$stub_bin/magikos-migrate" <<'SH'
#!/bin/bash
if [[ ${1:-} == "--pending" && ${MAGIKOS_TEST_PENDING_MIGRATIONS:-0} == 1 ]]; then
...
SH
chmod +x "$stub_bin/magikos-migrate"
...
cat >"$stub_bin/magikos-notification-wait" <<'SH'
#!/bin/bash
[[ ${MAGIKOS_TEST_LOCK_DURING_WAIT:-0} == 1 ]] || exit 0

lock="$XDG_RUNTIME_DIR/magikos-update.lock"
held="$lock.held"
rm -f "$held"
: >"$lock"
...
echo "$!" >"$MAGIKOS_TEST_LOCK_HOLDER_PID"
...
exit 1
SH
chmod +x "$stub_bin/magikos-notification-wait"

cat >"$stub_bin/magikos-notification-send" <<'SH'
#!/bin/bash
[[ ${MAGIKOS_TEST_NOTIFY_SEND:-send} == "fail" ]] && exit 1
printf '%s\n' "$@" >"$MAGIKOS_TEST_NOTIFY_ARGS"
SH
chmod +x "$stub_bin/magikos-notification-send"
...
run_notify() {
  ...
  MAGIKOS_TEST_PENDING_MIGRATIONS="$1" \
  MAGIKOS_TEST_NOTIFY_ARGS="$test_tmp/notify-args" \
  MAGIKOS_TEST_NOTIFY_SEND="${2:-send}" \
  MAGIKOS_TEST_LOCK_DURING_WAIT="${MAGIKOS_TEST_LOCK_DURING_WAIT:-0}" \
  MAGIKOS_TEST_LOCK_HOLDER_PID="$test_tmp/lock-holder-pid" \
    "$ROOT/bin/magikos-migrate-notify"
}
...
grep -q 'Magikos has pending migrations' "$test_tmp/pending.err" || fail "migration notifier explains pending migrations without notification system"
...
grep -Fx 'Pending Magikos Migrations' "$test_tmp/notify-args" >/dev/null || fail "migration notifier uses pending migrations title"
...
# `magikos update` applies migrations itself, so nothing may notify about them
# while it holds its lock -- a stale trigger firing mid-transaction is exactly
# how the retired magikos-update-user-notify.path used to interrupt updates.
rm -f "$test_tmp/notify-args"
update_lock="$runtime_dir/magikos-update.lock"
: >"$update_lock"
...
pass "migration notifier stays quiet while magikos update holds its lock"

exec {update_lock_fd}>&-

run_notify 1 >/dev/null 2>&1
notify_args_written &&
  grep -Fx 'Pending Magikos Migrations' "$test_tmp/notify-args" >/dev/null ||
...
rm -f "$test_tmp/notify-args"
MAGIKOS_TEST_LOCK_DURING_WAIT=1 run_notify 1 >"$test_tmp/raced.out" 2>"$test_tmp/raced.err"
if [[ -s $test_tmp/lock-holder-pid ]]; then
  kill "$(<"$test_tmp/lock-holder-pid")" 2>/dev/null || true
  for _ in {1..200}; do
    flock -n "$runtime_dir/magikos-update.lock" true 2>/dev/null && break
    sleep 0.05
  done
fi
...
rm -f "$test_tmp/notify-args" /tmp/magikos-update.lock
foreign_lock="$test_tmp/foreign/magikos-update.lock"
...
grep -Fx 'magikos-launch-floating-terminal-with-presentation magikos-migrate' "$test_tmp/notify-args" >/dev/null ||
  fail "migration notifier points the click command at magikos-migrate"
