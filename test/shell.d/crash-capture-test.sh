...
cat >"$TMPDIR/bin/magikos-notification-send" <<'SH'
#!/bin/bash
exit 0
SH

chmod +x "$TMPDIR/bin/systemctl" "$TMPDIR/bin/magikos-notification-send"

test_home="$TMPDIR/home"
flag="$test_home/.local/state/magikos/toggles/crash-capture-off"

toggle_crash_capture() {
  PATH="$TMPDIR/bin:$ROOT/bin:$PATH" \
  SYSTEMCTL_LOG="$SYSTEMCTL_LOG" \
  HOME="$test_home" \
    "$ROOT/bin/magikos-toggle-crash-capture"
}

toggle_crash_capture
[[ -f $flag ]] || fail "crash capture toggle disables the watcher"
grep -Fqx -- "--user stop magikos-crash-watch.service" "$SYSTEMCTL_LOG" ||
...
grep -Fqx -- "--user start magikos-crash-watch.service" "$SYSTEMCTL_LOG" ||
...
service="$ROOT/default/systemd/user/magikos-crash-watch.service"
grep -Fx 'ConditionPathExists=!%h/.local/state/magikos/toggles/crash-capture-off' "$service" >/dev/null ||
...
grep -F 'magikos-crash-watch.service' "$ROOT/install/user/first-run/enable-user-units.sh" >/dev/null ||
...
const items = menu.parseMenuJsonc(fs.readFileSync(path.join(root, 'default/magikos/magikos-menu.jsonc'), 'utf8'))
const byId = Object.fromEntries(items.map(item => [item.id, item]))

assertEqual(
  byId['trigger.toggle.crash-capture'].action,
  'magikos-toggle-crash-capture',
