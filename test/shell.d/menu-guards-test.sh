...
const items = {
  'setup.default.browser.brave': { id: 'setup.default.browser.brave', when: 'magikos-pkg-present brave-bin', checked: '[[ "$(magikos-default-browser)" == "brave" ]]' },
  'setup.default.browser.zen': { id: 'setup.default.browser.zen', when: 'magikos-pkg-present zen-browser-bin', checked: '[[ "$(magikos-default-browser)" == "zen" ]]' },
  'install.browser.zen': { id: 'install.browser.zen', disabled: 'magikos-pkg-present zen-browser-bin' },
  'plain': { id: 'plain', label: 'No guards' }
}
const script = menu.guardScript(items)
const browserSlot = `\${__magikos_read_${menu.guardReaders.indexOf('magikos-default-browser')}}`

assert(
  script.includes('if { magikos-pkg-present brave-bin; } >/dev/null 2>&1; then echo setup.default.browser.brave:w:1; else echo setup.default.browser.brave:w:0; fi'),
...
assert(
  script.includes('if { magikos-pkg-present zen-browser-bin; } >/dev/null 2>&1; then echo install.browser.zen:d:1; else echo install.browser.zen:d:0; fi'),
...
assertEqual(
  (script.match(/^__magikos_read_\d+=\$\(magikos-default-browser /gm) || []).length,
  1,
  'guard script reads a value command once for the whole batch'
)
assert(
  script.includes(`[[ "${browserSlot}" == "brave" ]]`) && !script.includes('"$(magikos-default-browser)"'),
  'guard script substitutes the captured answer into the expression'
)
assert(
  script.indexOf('__magikos_read_') < script.indexOf('if { magikos-pkg-present'),
...
const untouched = menu.guardScript({
  a: { id: 'a', when: 'command -v magikos-dns' },
  b: { id: 'b', when: '[[ "$(MAGIKOS_PATH=/usr/share/magikos magikos-channel-current)" == "stable" ]]' },
  c: { id: 'c', when: '(( $(magikos-default-browser | wc -l) == 1 ))' }
})
assert(
  untouched.includes('command -v magikos-dns')
    && untouched.includes('$(MAGIKOS_PATH=/usr/share/magikos magikos-channel-current)')
    && untouched.includes('$(magikos-default-browser | wc -l)'),
  'guard script leaves every form but the plain substitution to run the real command'
)
assert(
  !/^__magikos_read_/m.test(untouched),
...
const fs = require('fs')
const defaultItems = menu.parseMenuJsonc(fs.readFileSync(path.join(root, 'default/magikos/magikos-menu.jsonc'), 'utf8'))
const guardText = defaultItems.map(item => `${item.when}\n${item.checked}\n${item.disabled}`).join('\n')
const repeated = [...new Set(
  (guardText.match(/\$\((magikos-[a-z0-9-]+)\)/g) || []).map(match => match.slice(2, -1))
...
for helper in magikos-pkg-present magikos-pkg-missing; do
...
for helper in magikos-cmd-present magikos-cmd-missing; do
  for case in "${cmd_cases[@]}"; do
    read -r -a argv <<<"$case"
    assert_helper_agrees "guard prelude resolves commands as the real helper does" "$helper" "${argv[@]}"
  done
done
pass "guard prelude resolves commands as magikos-cmd-present and magikos-cmd-missing do"
...
reader_script=$(node -e '
  ...
  process.stdout.write(menu.guardScript({
    hit: { id: "hit", checked: "[[ \"$(magikos-dns)\" == \"Cloudflare\" ]]" },
    miss: { id: "miss", checked: "[[ \"$(magikos-dns)\" == \"Google\" ]]" }
  }))
')
reader_result=$(bash -c '
magikos-dns() { printf "Cloudflare\n"; }
export -f magikos-dns
'"$reader_script")
