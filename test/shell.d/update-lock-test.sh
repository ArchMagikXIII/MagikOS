...
# magikos-update should hold the lock before snapshotting, so a second update
# cannot even enter its pre-update snapshot.
update_snapshot_marker="$test_tmp/update-snapshot-started"
write_stub magikos-snapshot 'echo started >"$TEST_MARKER"; sleep 2; exit 0'

MAGIKOS_UPDATE_LOGGED=1 TEST_MARKER="$update_snapshot_marker" run_with_lock_env "$ROOT/bin/magikos-update" -y >"$test_tmp/update-first.out" 2>&1 &
update_pid=$!
...
[[ -f $update_snapshot_marker ]] || fail "first magikos-update reached snapshot under lock"

set +e
MAGIKOS_UPDATE_LOGGED=1 TEST_MARKER="$test_tmp/update-second-snapshot-started" run_with_lock_env "$ROOT/bin/magikos-update" -y >"$test_tmp/update-second.out" 2>&1
update_second_status=$?
set -e

wait "$update_pid"

[[ $update_second_status -ne 0 ]] || fail "second magikos-update exits non-zero while update lock is held"
grep -q "already running" "$test_tmp/update-second.out" || fail "second magikos-update reports held update lock"
[[ ! -f $test_tmp/update-second-snapshot-started ]] || fail "second magikos-update did not snapshot while lock was held"
pass "magikos-update prevents overlapping top-level updates"
...
# later update and silencing magikos-migrate-notify, which reads the same lock.
inhibit_pid_file="$test_tmp/inhibit-pid"
keyring_marker="$test_tmp/keyring-started"
write_stub magikos-snapshot 'exit 0'
write_stub systemd-inhibit 'echo "$$" >"$INHIBIT_PID_FILE"; exec sleep 30'
write_stub magikos-update-keyring 'echo started >"$TEST_MARKER"; sleep 3; exit 0'

MAGIKOS_UPDATE_LOGGED=1 TEST_MARKER="$keyring_marker" INHIBIT_PID_FILE="$inhibit_pid_file" \
  run_with_lock_env "$ROOT/bin/magikos-update" -y >"$test_tmp/update-inhibit.out" 2>&1 &
inhibit_update_pid=$!
...
lock_target=$(readlink -f "$runtime_dir/magikos-update.lock")
...
pass "magikos-update keeps the update lock out of its sleep inhibitor"

kill -0 "$inhibitor_pid" 2>/dev/null &&
  fail "update waits for its sleep inhibitor to stop before continuing"
pass "magikos-update waits for its sleep inhibitor to stop"
...
write_stub magikos-snapshot 'exit 0'
write_stub magikos-update-keyring 'exit 0'
write_stub magikos-toggle-idle '
state_file="$HOME/.local/state/magikos/indicators/stay-awake"
case "$1" in
  stay-awake)
...
write_stub magikos-update-restart '
state_file="$HOME/.local/state/magikos/indicators/stay-awake"
...
rm -f "$test_home/.local/state/magikos/indicators/stay-awake"
MAGIKOS_UPDATE_LOGGED=1 run_with_lock_env "$ROOT/bin/magikos-update" -y
[[ ! -f $test_home/.local/state/magikos/indicators/stay-awake ]] || fail "update clears its Stay Awake state before restart handling"

mkdir -p "$test_home/.local/state/magikos/indicators"
touch "$test_home/.local/state/magikos/indicators/stay-awake"
MAGIKOS_UPDATE_LOGGED=1 EXPECT_STAY_AWAKE=1 run_with_lock_env "$ROOT/bin/magikos-update" -y
[[ -f $test_home/.local/state/magikos/indicators/stay-awake ]] || fail "update preserves pre-existing Stay Awake state"
pass "magikos-update restores only its own Stay Awake state before restart handling"
