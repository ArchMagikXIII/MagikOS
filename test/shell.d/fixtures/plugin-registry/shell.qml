...
ShellRoot {
  ...
  readonly property string resultPath: Quickshell.env("MAGIKOS_QML_TEST_RESULT")
  ...
  function runChecks() {
    var scan = ""
    scan += block("firstparty", "/first/widgets/clock", manifest("magikos.first-widget", ["bar-widget"], { barWidget: "Widget.qml" }))
    scan += block("firstparty", "/first/bar", manifest("magikos.bar", ["bar"], { bar: "Bar.qml" }))
    scan += block("firstparty", "/first/panels/grouped", manifest("magikos.grouped-panel", ["panel"], { panel: "Panel.qml" }))
    scan += block("firstparty", "/first/hybrid", manifest("magikos.hybrid", ["menu", "bar-widget"], { menu: "Menu.qml", barWidget: "Widget.qml" }))
    ...
    localWidget.magikos = { clonedFrom: "magikos.first-widget" }
    ...
    localWeather.magikos = { clonedFrom: "magikos.weather" }
    ...
    root.assertDeepEqual(pluginIds(), [
      ...
      "local.weather",
      "magikos.bar",
      "magikos.first-widget",
      "magikos.grouped-panel",
      "magikos.hybrid",
      "third.bar",
      "third.center-widget",
      "third.panel",
    ...
    root.assertTrue(registry.installedPlugins["magikos.first-widget"].__isFirstParty === true, "first-party manifests are stamped")
    root.assertTrue(registry.installedPlugins["third.panel"].__isFirstParty === false, "third-party manifests are stamped")
    root.assertEqual(registry.installedPlugins["magikos.grouped-panel"].__sourceDir, "/first/panels/grouped", "grouped plugin source paths are preserved")
    ...
    root.assertTrue(!has("magikos.reserved"), "third-party magikos namespace ids are rejected")
    ...
    root.assertTrue(registry.isEnabled("magikos.first-widget"), "first-party plugins are implicitly enabled")
    root.assertTrue(registry.isEnabled("magikos.bar"), "built-in bar option is active by default")
    ...
    root.assertEqual(registry.resolveEnabledId("magikos.first-widget"), "magikos.first-widget", "inactive clones do not replace their source id")
    ...
    root.assertTrue(!registry.isEnabled("magikos.bar"), "selecting third-party bar options deactivates built-in bar")
    ...
    root.assertTrue(registry.isEnabled("magikos.bar"), "built-in bar option returns after reset")
    ...
    root.config = {
      ...
      bar: {
        layout: {
          left: [{ id: "magikos.workspaces" }, { id: "magikos.menu" }],
          center: [{ id: "magikos.weather" }, { id: "magikos.clock" }],
          right: [{ id: "magikos.tray" }]
        }
      },
      plugins: []
    }
    ...
    root.assertDeepEqual(
      ...
      [{ id: "magikos.workspaces" }, { id: "third.widget" }, { id: "magikos.menu" }],
    ...
    root.assertDeepEqual(
      ...
      [{ id: "magikos.weather" }, { id: "third.center-widget" }, { id: "magikos.clock" }],
    ...
    root.assertDeepEqual(
      ...
      [{ id: "magikos.tray" }, { id: "third.right-widget" }],
    ...
    root.config = {
      version: 1,
      bar: { layout: { left: [], center: [{ id: "magikos.weather" }], right: [] } },
      plugins: []
    }
    root.assertTrue(
      !registry.setEnabled("third.center-widget", true, { after: "magikos.first-widget" }),
      "enabling against a widget the bar does not carry is refused"
    )
    root.assertEqual(
      registry.lastEnableError,
      "could not find target widget magikos.first-widget",
      "a refused enable names the target it could not find"
    )
    root.assertDeepEqual(root.config.bar.layout.center, [{ id: "magikos.weather" }], "a refused enable places nothing")
    root.assertEqual(
      registry.putBarWidget("third.center-widget", { after: "magikos.first-widget" }),
      "",
      "put accepts a placement target the bar does not carry"
    )
    root.assertDeepEqual(
      root.config.bar.layout.center,
      [{ id: "magikos.weather" }, { id: "third.center-widget" }],
    ...
    root.config = {
      version: 1,
      bar: { layout: { left: [], center: [{ id: "local.first-widget" }, { id: "magikos.weather" }], right: [] } },
      plugins: []
    }
    root.assertEqual(
      registry.putBarWidget("third.center-widget", { after: "magikos.first-widget" }),
    ...
    root.assertDeepEqual(
      root.config.bar.layout.center,
      [{ id: "local.first-widget" }, { id: "third.center-widget" }, { id: "magikos.weather" }],
    ...
    root.config = {
      version: 1,
      bar: { layout: { left: [], center: [{ id: "local.weather" }, { id: "magikos.clock" }], right: [] } },
      plugins: []
    }
    root.assertEqual(
      registry.putBarWidget("third.center-widget", { after: "magikos.first-widget" }),
    ...
    root.assertDeepEqual(
      root.config.bar.layout.center,
      [{ id: "local.weather" }, { id: "third.center-widget" }, { id: "magikos.clock" }],
    ...
    root.assertEqual(registry.putBarWidget("magikos.first-widget", {}), "", "put accepts a widget whose clone is on the bar")
    root.assertDeepEqual(
      root.config.bar.layout.center,
    ...
    root.config = {
      version: 1,
      bar: { layout: { left: [], center: [{ id: "magikos.first-widget", size: 4 }], right: [] } },
      plugins: []
    }
    ...
    root.assertEqual(registry.resolveEnabledId("magikos.first-widget"), "local.first-widget", "enabled clones receive calls made to their source id")
    registry.setEnabled("magikos.first-widget", true)
    root.assertDeepEqual(
      root.config.bar.layout.center,
      [{ id: "magikos.first-widget", size: 4 }],
    ...
    root.config = {
      version: 1,
      bar: { layout: { left: [], center: ["magikos.first-widget"], right: [] } },
      plugins: []
    }
    registry.setEnabled("local.first-widget", true)
    root.assertDeepEqual(root.config.bar.layout.center, [{ id: "local.first-widget" }], "clone replacement normalizes string entries")
    registry.setEnabled("local.first-widget", false)
    root.assertDeepEqual(root.config.bar.layout.center, [{ id: "magikos.first-widget" }], "clone restoration normalizes string entries")

    root.config = {
      version: 1,
      bar: { layout: { left: [{ id: "magikos.hybrid" }], center: [], right: [] } },
      plugins: []
    }
    ...
    root.assertDeepEqual(root.config.disabledPlugins, ["magikos.hybrid"], "enabling a multi-kind clone disables the source")
    registry.setEnabled("local.hybrid", false)
    root.assertDeepEqual(root.config.bar.layout.left, [{ id: "magikos.hybrid" }], "disabling a multi-kind clone restores its widget")
    ...
    root.assertDeepEqual(root.config.disabledPlugins, ["magikos.grouped-panel"], "enabling an ordinary clone disables the source")
    ...
    root.config = {
      ...
      bar: { layout: { left: [], center: [], right: [] } },
      plugins: [],
      disabledPlugins: ["magikos.grouped-panel"]
    }
    registry.setEnabled("local.grouped-panel", true)
    root.assertDeepEqual(root.config.disabledPlugins, ["magikos.grouped-panel"], "cloning an already-disabled source keeps it disabled")
    root.assertTrue(root.config.cloneSourceRestores === undefined, "an already-disabled source is not marked for restoration")
    registry.setEnabled("local.grouped-panel", false)
    root.assertDeepEqual(root.config.disabledPlugins, ["magikos.grouped-panel"], "disabling the clone preserves the source's prior disabled state")
    ...
    root.config = { version: 1, bar: { layout: { left: [], center: [], right: [] } }, plugins: [] }
    registry.setEnabled("magikos.grouped-panel", false)
    root.assertDeepEqual(root.config.disabledPlugins, ["magikos.grouped-panel"], "disabling a first-party plugin records it")
    root.assertTrue(!registry.isEnabled("magikos.grouped-panel"), "a recorded first-party plugin is disabled")
    root.assertDeepEqual(root.config.plugins, [], "disabling a first-party plugin leaves the plugins array alone")
    registry.setEnabled("magikos.grouped-panel", true)
    root.assertTrue(root.config.disabledPlugins === undefined, "re-enabling drops the disabled record entirely")
    root.assertTrue(registry.isEnabled("magikos.grouped-panel"), "a first-party plugin returns to enabled")
    ...
    // follow it down, or a plugin that is both widget and menu (magikos.menu)
    // would be locked out of the shell by taking its button off the bar.
    root.config = {
      version: 1,
      bar: { layout: { left: [], center: [], right: [{ id: "magikos.first-widget" }] } },
      plugins: []
    }
    root.assertTrue(registry.inBar("magikos.first-widget"), "inBar sees a widget in the layout")
    registry.setEnabled("magikos.first-widget", false)
    ...
    root.assertTrue(!registry.inBar("magikos.first-widget"), "inBar follows the widget out of the layout")
    root.assertTrue(registry.isEnabled("magikos.first-widget"), "a first-party widget stays loadable off the bar")
    registry.setEnabled("magikos.first-widget", true)
    root.assertDeepEqual(root.config.bar.layout.center, [{ id: "magikos.first-widget" }], "a widget without a default section falls back to center")
    ...
    registry.setEnabled("magikos.hybrid", false)
    ...
    root.assertTrue(registry.isEnabled("magikos.hybrid"), "a multi-kind built-in remains loadable without its widget")
  ...
  PluginRegistry {
    id: registry
    firstPartyDir: ""
    pluginsDir: Quickshell.env("HOME") + "/.config/magikos/plugins"
