...
#!/bin/bash
[[ ! -e $MAGIKOS_TEST_INSTALLED_DIR/$1 ]]
SH

cat >"$mock_bin/magikos-launch-floating-terminal-with-presentation" <<'SH'
#!/bin/bash
printf '%s\0' "$@" >"$MAGIKOS_TEST_TERMINAL_LOG"
SH

cat >"$mock_bin/magikos-notification-send" <<'SH'
#!/bin/bash
printf '%s\0' "$@" >>"$MAGIKOS_TEST_NOTIFICATION_LOG"
SH

cat >"$mock_bin/magikos-test-setup-call" <<'SH'
#!/bin/bash
printf '%s:%s\n' "${0##*/}" "$*" >>"$MAGIKOS_TEST_SETUP_LOG"
[[ ${MAGIKOS_TEST_SETUP_FAIL:-} != "${0##*/}" ]]
SH

cat >"$mock_bin/sudo" <<'SH'
#!/bin/bash
printf 'sudo:%s\n' "$*" >>"$MAGIKOS_TEST_SETUP_LOG"
[[ ${MAGIKOS_TEST_SETUP_FAIL:-} != "sudo" ]]
SH

cat >"$mock_bin/xdg-settings" <<'SH'
#!/bin/bash
case $1 in
get) [[ -f $MAGIKOS_TEST_BROWSER_FILE ]] && cat "$MAGIKOS_TEST_BROWSER_FILE" ;;
set) printf '%s\n' "$3" >"$MAGIKOS_TEST_BROWSER_FILE" ;;
esac
SH

cat >"$mock_bin/magikos-test-installer" <<'SH'
#!/bin/bash
installer=${0##*/}

if [[ $installer == "magikos-install-browser" && ${MAGIKOS_TEST_REAL_BROWSER_INSTALL:-false} == "true" ]]; then
  exec "$ROOT/bin/magikos-install-browser" "$@"
fi

case $installer in
magikos-pkg-add)
  package=$1
  printf 'pkg:%s\n' "$package" >>"$MAGIKOS_TEST_INSTALL_LOG"
  case $package in
  ...
  ;;
magikos-install-browser)
  selection=$1
  printf 'browser:%s\n' "$selection" >>"$MAGIKOS_TEST_INSTALL_LOG"
  case $selection in
  chromium) command=chromium ;;
  ...
  ;;
magikos-install-terminal)
  command=$1
  printf 'terminal:%s\n' "$command" >>"$MAGIKOS_TEST_INSTALL_LOG"
  ;;
magikos-install-editor-*)
  editor=${installer#magikos-install-editor-}
  printf 'editor:%s\n' "$editor" >>"$MAGIKOS_TEST_INSTALL_LOG"
  case $editor in
  vscode) command=code ;;
  zed) command=zeditor ;;
  helix) command=helix ;;
...
[[ ${MAGIKOS_TEST_INSTALL_FAIL:-false} != "true" ]] || exit 1
touch "$MAGIKOS_TEST_INSTALLED_DIR/$command"
SH
...
for installer in \
  ...
  magikos-install-browser \
  magikos-install-terminal \
  magikos-install-editor-vscode \
  magikos-install-editor-zed \
  magikos-install-editor-helix \
  magikos-install-editor-emacs; do
  ln -s magikos-test-installer "$mock_bin/$installer"
done
for setup_command in \
  magikos-install-chromium-copy-url \
  magikos-install-chromium-ytdlp \
  magikos-theme-set-browser; do
  ln -s magikos-test-setup-call "$mock_bin/$setup_command"
done
...
export MAGIKOS_TEST_INSTALLED_DIR="$installed_dir"
export MAGIKOS_TEST_INSTALL_LOG="$install_log"
export MAGIKOS_TEST_TERMINAL_LOG="$terminal_log"
export MAGIKOS_TEST_NOTIFICATION_LOG="$notification_log"
export MAGIKOS_TEST_SETUP_LOG="$setup_log"
export MAGIKOS_TEST_BROWSER_FILE="$browser_file"
...
assert_missing_opens_installer() {
  ...
  : >"$terminal_log"
  "magikos-default-$type" "$selection"
  mapfile -d '' -t terminal_args <"$terminal_log"
  [[ ${terminal_args[*]} == "magikos-default-$type --install $selection" ]] ||
...
for entry in "${browser_cases[@]}"; do
  ...
  : >"$install_log"
  magikos-default-browser --install "$selection"
  [[ $(<"$install_log") == "$installer" ]] || fail "$selection uses its browser installer"
  [[ $(magikos-default-browser) == "$selection" ]] || fail "$selection becomes the default browser after installation"
done
...
MAGIKOS_TEST_REAL_BROWSER_INSTALL=true magikos-default-browser --install chromium >/dev/null
[[ $(<"$install_log") == "pkg:chromium" ]] || fail "Chromium browser installer installs the package"
[[ $(magikos-default-browser) == "chromium" ]] || fail "Chromium becomes the default after its full installer succeeds"
...
grep -Fxq 'magikos-install-chromium-copy-url:' "$setup_log" ||
  fail "Chromium browser installer registers the Copy URL host"
grep -Fxq 'magikos-install-chromium-ytdlp:' "$setup_log" ||
  fail "Chromium browser installer registers the yt-dlp host"
grep -Fxq 'magikos-theme-set-browser:' "$setup_log" ||
  fail "Chromium browser installer applies the current theme"
pass "Chromium browser installer restores the complete Magikos setup"

magikos-default-browser zen
rm -f "$installed_dir/chromium"
if MAGIKOS_TEST_REAL_BROWSER_INSTALL=true MAGIKOS_TEST_INSTALL_FAIL=true \
  magikos-default-browser --install chromium >"$test_tmp/browser-package-failure" 2>&1; then
  fail "failed Chromium package installation returns an error"
fi
[[ $(magikos-default-browser) == "zen" ]] || fail "failed Chromium package installation preserves the default browser"
...
if MAGIKOS_TEST_REAL_BROWSER_INSTALL=true MAGIKOS_TEST_SETUP_FAIL=sudo \
  magikos-default-browser --install chromium >"$test_tmp/browser-install-failure" 2>&1; then
  fail "failed Chromium setup returns an error"
fi
[[ $(magikos-default-browser) == "zen" ]] || fail "failed Chromium setup preserves the default browser"
...
for entry in "${terminal_cases[@]}"; do
  ...
  : >"$install_log"
  magikos-default-terminal --install "$selection"
...
for entry in "${editor_cases[@]}"; do
  ...
  rm -f "$installed_dir/$command"
  : >"$install_log"
  magikos-default-editor --install "$selection"
  [[ $(<"$install_log") == "$installer" ]] || fail "$selection uses its editor installer"
  [[ $(magikos-default-editor) == "$command" ]] || fail "$selection becomes the default editor after installation"
done
...
: >"$terminal_log"
magikos-default-browser zen
magikos-default-terminal kitty
magikos-default-editor nvim
[[ ! -s $install_log && ! -s $terminal_log ]] || fail "installed defaults skip installation"
pass "installed defaults are selected immediately"

previous_editor=$(magikos-default-editor)
rm -f "$installed_dir/vim"
if MAGIKOS_TEST_INSTALL_FAIL=true magikos-default-editor --install vim >"$test_tmp/install-failure" 2>&1; then
  fail "failed default installation returns an error"
fi
[[ $(magikos-default-editor) == "$previous_editor" ]] || fail "failed installation preserves the default"
