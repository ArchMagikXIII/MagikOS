...
scale_log="$home_dir/.local/state/magikos/monitor-scaling.log"
...
if [[ $1 == "monitors" && $2 == "-j" ]]; then
  ...
  printf '%s\n' "$2" >"$MAGIKOS_TEST_HYPRCTL_EVAL_OUT"
else
  exit 1
fi
SH
...
run_scaling() {
  HOME="$home_dir" \
    ...
    PATH="$stub_bin:$PATH" \
    MAGIKOS_TEST_HYPRCTL_EVAL_OUT="$eval_out" \
    MAGIKOS_TEST_MONITOR_SCALE="${MAGIKOS_TEST_MONITOR_SCALE:-2}" \
    "$ROOT/bin/magikos-hyprland-monitor-scaling" "$@"
}

write_monitor_config
MAGIKOS_TEST_MONITOR_SCALE=2 run_scaling up
grep -F 'scale = 3' "$eval_out" >/dev/null || fail "monitor scaling up reaches 3x"
grep -Fx 'local magikos_monitor_scale = 3' "$monitor_lua" >/dev/null || fail "monitor scaling up persists 3x"
...
grep -Fx 'local magikos_monitor_scale = 2' "$monitor_lua" >/dev/null || fail "monitor scaling down persists 2x from 3x"
...
grep -Fx 'local magikos_monitor_scale = 2' "$monitor_lua" >/dev/null || fail "monitor scaling down persists 2x from floating point 3x"
...
grep -Fx 'local magikos_monitor_scale = 3' "$monitor_lua" >/dev/null || fail "monitor scaling explicit 3x persists"
grep -Fx 'local magikos_gdk_scale = 3' "$monitor_lua" >/dev/null || fail "monitor scaling explicit 3x persists GDK scale"
...
grep -Fx 'local magikos_monitor_scale = 1.6' "$monitor_lua" >/dev/null || fail "monitor scaling explicit 1.6x persists"
grep -Fx 'local magikos_gdk_scale = 2' "$monitor_lua" >/dev/null || fail "monitor scaling 1.6x persists integer GDK scale 2"
pass "monitor scaling 1.6x persists integer GDK scale 2"

write_monitor_config
MAGIKOS_TEST_MONITOR_SCALE=2 run_scaling 1.25
grep -Fx 'local magikos_monitor_scale = 1.25' "$monitor_lua" >/dev/null || fail "monitor scaling explicit 1.25x persists"
grep -Fx 'local magikos_gdk_scale = 1' "$monitor_lua" >/dev/null || fail "monitor scaling 1.25x persists integer GDK scale 1"
...
grep -Fx 'local magikos_monitor_scale = 3.2' "$monitor_lua" >/dev/null ||
...
grep -Fx 'local magikos_monitor_scale = 2' "$monitor_lua" >/dev/null ||
