...
export MAGIKOS_POWERPROFILES_STATE_DIR="$tmp_dir/state"

"$ROOT/bin/magikos-powerprofiles-set" ac balanced
...
"$ROOT/bin/magikos-powerprofiles-set" ac
[[ $(tail -n 1 "$tmp_dir/calls") == "balanced" ]] || fail "power profile restores AC preference"
pass "power profile restores AC preference"

if POWERPROFILES_SET_FAIL=1 "$ROOT/bin/magikos-powerprofiles-set" ac performance; then
...
"$ROOT/bin/magikos-powerprofiles-set" battery performance
...
ON_BATTERY=1 "$ROOT/bin/magikos-powerprofiles-set"
...
rm "$tmp_dir/state/ac"
ON_BATTERY=0 "$ROOT/bin/magikos-powerprofiles-set"
...
"$ROOT/bin/magikos-powerprofiles-set" ac power-saver
"$ROOT/bin/magikos-powerprofiles-init"
...
rg -F '["magikos-powerprofiles-set", pendingPowerSource]' "$ROOT/shell/plugins/services/battery/Service.qml" >/dev/null ||
  fail "battery service applies profiles through Magikos command"
pass "battery service applies profiles through Magikos command"

rg -F 'magikos-powerprofiles-set autodetect' "$ROOT/shell/plugins/menu/Menu.qml" >/dev/null ||
  fail "power profile menu persists selections through Magikos command"
pass "power profile menu persists selections through Magikos command"
