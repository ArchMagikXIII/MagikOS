...
#!/bin/bash
printf 'pkg:%s\n' "$*" >>"$MAGIKOS_TEST_LOG"
exit "${MAGIKOS_TEST_PKG_STATUS:-0}"
SH

for command in magikos-pkg-aur-add magikos-install-emacs omazed magikos-theme-set-vscode magikos-install-gaming-gpu-lib32; do
...
#!/bin/bash
printf '%s\n' "$1" >"$MAGIKOS_TEST_PRESENTATION"
SH

chmod +x "$mock_bin"/*

export HOME="$test_home"
export MAGIKOS_TEST_LOG="$test_tmp/launch.log"
export MAGIKOS_TEST_PRESENTATION="$test_tmp/presentation"
...
wait_for_launch() {
  ...
  for ((attempt = 0; attempt < 100; attempt++)); do
    grep -Fxq "$expected" "$MAGIKOS_TEST_LOG" && return 0
    sleep 0.01
  done
...
assert_detached_installer_launch magikos-install-editor-emacs emacsclient
assert_detached_installer_launch magikos-install-editor-vscode code
assert_detached_installer_launch magikos-install-editor-zed dev.zed.Zed
assert_detached_installer_launch magikos-install-gaming-heroic heroic
assert_detached_installer_launch magikos-install-gaming-steam steam

bash "$ROOT/bin/magikos-install-and-launch" "Example App" "alpha beta" "Disk Usage"
presentation_command=$(<"$MAGIKOS_TEST_PRESENTATION")
...
[[ $presentation_command == *'magikos-pkg-add alpha beta && (setsid uwsm-app -- gtk-launch Disk\ Usage >/dev/null 2>&1 &)'* ]] ||
...
grep -Fxq 'pkg:alpha beta' "$MAGIKOS_TEST_LOG" ||
...
: >"$MAGIKOS_TEST_LOG"
if MAGIKOS_TEST_PKG_STATUS=1 bash -c "$presentation_command"; then
  fail "generic installer propagates package installation failure"
fi
if grep -q '^launch:' "$MAGIKOS_TEST_LOG"; then
