...
cat >"$mock_bin/magikos-done" <<'SH'
#!/bin/bash
[[ $1 == "check" && $2 == "first-run-user" ]]
SH
cat >"$mock_bin/magikos-provision-user" <<'SH'
#!/bin/bash
touch "$MAGIKOS_TEST_FINALIZE_CALLED"
SH
chmod +x "$mock_bin/magikos-done" "$mock_bin/magikos-provision-user"

finalize_called="$test_tmp/finalize-called"
HOME="$test_tmp/home" PATH="$mock_bin:$PATH" MAGIKOS_TEST_FINALIZE_CALLED="$finalize_called" \
  bash "$ROOT/bin/magikos-provision-first-run" >"$test_tmp/output"
...
if grep -F 'user-migration-notify-watch-enabled' "$ROOT/bin/magikos-provision-first-run" >/dev/null; then
