...
state_dir="$home_dir/.local/state/magikos/toggles/hypr"
...
if [[ $1 == "monitors" && $2 == "all" && $3 == "-j" ]]; then
  if [[ ${MAGIKOS_TEST_INTERNAL_DISABLED:-false} == "true" ]]; then
    printf '[{"name":"eDP-1","disabled":true,"scale":null}]'
  else
    printf '[{"name":"eDP-1","disabled":false,"scale":%s}]' "${MAGIKOS_TEST_INTERNAL_SCALE:-2}"
  fi
elif [[ $1 == "eval" ]]; then
  printf '%s\n' "$2" >>"$MAGIKOS_TEST_HYPRCTL_EVAL_LOG"
elif [[ $1 == "reload" ]]; then
  printf 'reload\n' >>"$MAGIKOS_TEST_HYPRCTL_EVAL_LOG"
...
cat >"$stub_bin/magikos-hyprland-monitor-internal" <<'SH'
#!/bin/bash
exit 0
SH

cat >"$stub_bin/magikos-hyprland-monitor-internal-mirror" <<'SH'
#!/bin/bash
exit 0
SH
...
cat >"$stub_bin/magikos-hw-clamshell" <<'SH'
#!/bin/bash
[[ ${MAGIKOS_TEST_CLAMSHELL:-false} == "true" ]]
SH
...
write_auto_monitor_config() {
  cat >"$monitor_lua" <<'LUA'
local magikos_gdk_scale = 2
local magikos_monitor_scale = "auto"
LUA
}
...
# A specific-output rule that references the magikos_monitor_scale variable
# (the default template's pattern) instead of a literal value. The variable
# must be resolved, not captured as the literal string "magikos_monitor_scale".
write_internal_monitor_var_config() {
  cat >"$monitor_lua" <<'LUA'
local magikos_gdk_scale = 1.5
local magikos_monitor_scale = 1.5
hl.env("GDK_SCALE", tostring(magikos_gdk_scale))
hl.monitor({ output = "eDP-1", mode = "preferred", position = "auto", scale = magikos_monitor_scale })
LUA
}

# An explicit non-numeric scale on the internal rule, alongside an unrelated
# magikos_monitor_scale local that must not be substituted for it.
write_internal_monitor_auto_config() {
  cat >"$monitor_lua" <<'LUA'
local magikos_monitor_scale = 2
...
# references the magikos_monitor_scale local. Quoted here, and commented below.
write_catch_all_var_config() {
  cat >"$monitor_lua" <<'LUA'
local magikos_monitor_scale = "1.5"
hl.monitor({ output = "", mode = "preferred", position = "auto", scale = magikos_monitor_scale })
LUA
}

write_commented_catch_all_var_config() {
  cat >"$monitor_lua" <<'LUA'
local magikos_monitor_scale = 1.25 -- HiDPI panel
hl.monitor({ output = "", mode = "preferred", position = "auto", scale = magikos_monitor_scale })
LUA
}
...
write_internal_monitor_position_var_config() {
  cat >"$monitor_lua" <<'LUA'
local magikos_monitor_position = "0x0"
hl.monitor({ output = "eDP-1", mode = "preferred", position = magikos_monitor_position, scale = 1.25 })
LUA
}
...
write_expression_scale_config() {
  cat >"$monitor_lua" <<'LUA'
local magikos_monitor_scale = 3 / 2
hl.monitor({ output = "", mode = "preferred", position = "auto", scale = magikos_monitor_scale })
LUA
}
...
run_clamshell() {
  HOME="$home_dir" \
    PATH="$stub_bin:$PATH" \
    MAGIKOS_TEST_HYPRCTL_EVAL_LOG="$eval_log" \
    MAGIKOS_TEST_INTERNAL_SCALE="${MAGIKOS_TEST_INTERNAL_SCALE:-2}" \
    MAGIKOS_TEST_INTERNAL_DISABLED="${MAGIKOS_TEST_INTERNAL_DISABLED:-false}" \
    MAGIKOS_TEST_EXTERNAL_ACTIVE="${MAGIKOS_TEST_EXTERNAL_ACTIVE:-false}" \
    MAGIKOS_TEST_CLAMSHELL="${MAGIKOS_TEST_CLAMSHELL:-false}" \
    "$ROOT/bin/magikos-hyprland-monitor-clamshell"
}

write_auto_monitor_config
: >"$eval_log"
MAGIKOS_TEST_INTERNAL_SCALE=3 run_clamshell
...
write_auto_monitor_config
: >"$eval_log"
MAGIKOS_TEST_INTERNAL_SCALE=2 run_clamshell
...
write_auto_monitor_config
: >"$eval_log"
MAGIKOS_TEST_INTERNAL_SCALE=1.6 MAGIKOS_TEST_EXTERNAL_ACTIVE=true MAGIKOS_TEST_CLAMSHELL=true run_clamshell
...
: >"$eval_log"
MAGIKOS_TEST_INTERNAL_DISABLED=true run_clamshell
...
# Regression: specific-output rule referencing the magikos_monitor_scale
# variable must resolve to the variable's value, not fall back to the default.
write_internal_monitor_var_config
rm -f "$scale_state"
: >"$eval_log"
MAGIKOS_TEST_INTERNAL_DISABLED=true run_clamshell
grep -F 'scale = 1.5' "$eval_log" >/dev/null || fail "clamshell recovery resolves magikos_monitor_scale variable reference"
pass "clamshell recovery resolves magikos_monitor_scale variable reference"

# An explicit "auto" on the internal rule is a value, not a variable reference,
# so it must not pick up an unrelated magikos_monitor_scale local.
write_internal_monitor_auto_config
remember_scale 1.75
: >"$eval_log"
MAGIKOS_TEST_INTERNAL_DISABLED=true run_clamshell
grep -F 'scale = 1.75' "$eval_log" >/dev/null || fail "clamshell recovery keeps auto scale off the magikos_monitor_scale local"
pass "clamshell recovery keeps auto scale off the magikos_monitor_scale local"
