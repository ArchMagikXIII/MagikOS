...
HOME="$test_home" "$ROOT/bin/magikos-toggle-idle" stay-awake >/dev/null
[[ -f $test_home/.local/state/magikos/indicators/stay-awake ]] || fail "Stay Awake toggle persists enabled state"

HOME="$test_home" "$ROOT/bin/magikos-toggle-idle" allow-idle >/dev/null
[[ ! -f $test_home/.local/state/magikos/indicators/stay-awake ]] || fail "Stay Awake toggle persists disabled state"

if rg -q 'magikos-shell' "$ROOT/bin/magikos-toggle-idle"; then
