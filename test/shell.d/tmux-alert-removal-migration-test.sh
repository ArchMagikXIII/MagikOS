...
case "$1" in
  ...
  show-hooks)
    cat <<'HOOKS'
after-select-window[0] run-shell -b "magikos-tmux-alert track #{window_id} #{window_activity}"
alert-activity
alert-bell[0] run-shell -b "magikos-shell -q magikos.indicators refresh"
client-focus-in[100] run-shell -b "magikos-tmux-alert track #{window_id} #{window_activity}"
...
cat >"$test_dir/bin/magikos-restart-tmux" <<'STUB'
#!/bin/bash

echo restart >>"$TMUX_RESTARTS"
STUB

cat >"$test_dir/bin/magikos-restart-shell" <<'STUB'
#!/bin/bash
...
cat >"$test_dir/bin/magikos-state" <<'STUB'
#!/bin/bash
...
shell_config="$home/.config/magikos/shell.json"
...
reset_home() {
  rm -rf "$home"
  mkdir -p "$home/.config/tmux" "$home/.config/magikos"
}
...
awk '
  /^# Status bar$/ {
    print "# Alerts"
    print "set-hook -g alert-bell '\''run-shell -b \"magikos-shell -q magikos.indicators refresh\"'\''"
    print "set-hook -g alert-activity '\''run-shell -b \"magikos-shell -q magikos.indicators refresh\"'\''"
    print "set-hook -g alert-silence '\''run-shell -b \"magikos-shell -q magikos.indicators refresh\"'\''"
    print "set-hook -g after-select-window '\''run-shell -b \"magikos-tmux-alert track #{window_id} #{window_activity}\"'\''"
    print "set-hook -g client-session-changed '\''run-shell -b \"magikos-tmux-alert track #{window_id} #{window_activity}\"'\''"
    print "set-hook -g client-focus-out[100] '\''run-shell -b \"magikos-tmux-alert track #{window_id} #{window_activity}\"'\''"
    print "set-hook -g client-focus-in[100] '\''run-shell -b \"magikos-tmux-alert track #{window_id} #{window_activity}\"'\''"
    print ""
  }
  { print }
...
# Alerts
set-hook -g alert-bell 'run-shell -b "magikos-shell -q magikos.indicators refresh"'
set-hook -g alert-activity 'run-shell -b "magikos-shell -q magikos.indicators refresh"'
set-hook -g after-select-window 'run-shell -b "magikos-shell -q magikos.indicators refresh"'
  set-hook -g client-session-changed 'run-shell -b "magikos-tmux-alert track #{window_id} #{window_activity}"'
set-hook -g client-focus-out[100] 'run-shell -b "magikos-tmux-alert track #{window_id} #{window_activity}"'
EOF
...
# is still the hook Magikos wrote.
...
grep -Fxq 'set-option -wqu -t @0 @magikos_unfocused_activity' "$TMUX_CALLS" || fail "alert removal clears tracked window options"
grep -Fxq 'set-option -wqu -t @3 @magikos_unfocused_activity' "$TMUX_CALLS" || fail "alert removal clears every tracked window"
...
cat >"$shell_config" <<'EOF'
{
  "bar": {
    "layout": {
      "left": [
        { "id": "magikos.indicators", "items": ["TmuxAlert", "Dnd", { "id": "TmuxAlert" }, { "id": "NightLight" }] }
      ],
      "center": [
        { "id": "magikos.indicators", "items": ["TmuxAlert"] },
        { "id": "magikos.clock" }
      ],
      "right": [
        { "id": "magikos.indicators", "indicators": ["TmuxAlert", "Dnd"] },
        { "id": "magikos.indicators", "items": [] }
      ]
    }
  }
}
...
[[ $(jq -c '[.bar.layout.center[].id]' "$shell_config") == '["magikos.clock"]' ]] ||
...
[[ $(jq -c '.bar.layout.right[1]' "$shell_config") == '{"id":"magikos.indicators","items":[]}' ]] ||
