...
sleep_service="$ROOT/default/systemd/user/magikos-sleep-lock.service"
grep -Fx 'ExecStart=/usr/bin/magikos-system-sleep-monitor' "$sleep_service" >/dev/null
...
grep -Fx 'ConditionEnvironment=MAGIKOS_PATH' "$sleep_service" >/dev/null ||
  fail "sleep lock monitor can start without the Magikos shell path"
...
grep -F 'magikos-sleep-lock.service' "$first_run_units" >/dev/null
...
grep -F 'ExecStart=%h/.local/share/magikos/bin/magikos-system-sleep-monitor' "$upgrade_to_quattro" >/dev/null
grep -F 'ExecStart=/usr/bin/magikos-system-sleep-monitor' "$upgrade_to_quattro" >/dev/null
grep -F 'reset-failed magikos-sleep-lock.service' "$upgrade_to_quattro" >/dev/null
pass "Magikos 4 upgrade repairs the legacy sleep lock unit path"

[[ -e $ROOT/default/systemd/user/magikos-update-user-notify.path ]] &&
  fail "the retired migration watcher is back; pacman writing the migration directory during magikos update would notify about migrations that update is already applying"
grep -rlE '^(Path[A-Za-z]+|DirectoryNotEmpty)=.*/usr/share/magikos/migrations' "$ROOT/default/systemd/user" >/dev/null 2>&1 &&
...
notify_service="$ROOT/default/systemd/user/magikos-migrate-notify.service"
grep -Fx 'ExecStart=/usr/bin/magikos-migrate-notify' "$notify_service" >/dev/null
...
grep -F 'magikos-migrate-notify.service' "$first_run_units" >/dev/null ||
  fail "first-run does not enable the login migration notifier"
grep -F 'magikos-update-user-notify' "$first_run_units" >/dev/null &&
  fail "first-run still enables the retired notifier units"
pass "first-run enables the login-only migration notifier"

fcitx_service="$ROOT/default/systemd/user/magikos-fcitx5.service"
...
grep -F 'systemctl --user enable magikos-fcitx5.service' "$fcitx_migration" >/dev/null ||
  fail "migration must enable without --now; --now starts the unit before the session-gate check"
grep -F 'Could not start magikos-fcitx5.service' "$fcitx_migration" >/dev/null ||
  fail "migration pkills a working fcitx5, so a failed handover must be reported instead of marked complete"

grep -F 'pkill -x fcitx5' "$ROOT/bin/magikos-restart-xcompose" >/dev/null ||
...
oomd_conf="$ROOT/etc/systemd/oomd.conf.d/10-magikos.conf"
