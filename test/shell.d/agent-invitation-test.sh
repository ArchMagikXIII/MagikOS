...
test_bin=$(mktemp -d)
log_file=$(mktemp)
hook_path="$test_home/.config/magikos/hooks/post-update.d/setup-agent.hook"
...
[[ -f $test_home/.local/state/magikos/done/agent-setup-invitation ]] || fail "agent invitation records completion"
...
grep -qx 'exec:magikos menu summon setup.default.agent' "$log_file" ||
...
fresh_home=$(mktemp -d)
mkdir -p "$fresh_home/.config/magikos/defaults"
printf 'claude\n' >"$fresh_home/.config/magikos/defaults/agent"
: >"$log_file"
cp "$ROOT/install/user/first-run/setup-agent.hook" "$hook_path"
...
[[ ! -f $fresh_home/.local/state/magikos/done/agent-setup-invitation ]] ||
