...
hybrid_gpu() {
  PATH="$fake_bin:$ROOT/bin:$PATH" timeout --kill-after=1s 10s bash "$ROOT/bin/magikos-hw-hybrid-gpu"
}
...
cat >"$fake_bin/magikos-cmd-present" <<'STUB'
#!/bin/bash
exit 1
STUB
chmod +x "$fake_bin/magikos-cmd-present"
