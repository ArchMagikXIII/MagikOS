...
`test/acceptance.d/*-test.sh`. It exercises a real installed Magikos desktop,
...
Run acceptance tests in a disposable VM through the sibling `magikos-iso`
...
```bash
cd ../magikos-iso
./bin/magikos-iso-test release/<iso>.iso --reuse-base --sync-magikos ../magikos --no-preview
```

Use `--sync-all ../magikos` instead of `--sync-magikos ../magikos` when the
...
```bash
cd ../magikos-iso
./bin/magikos-iso-make --no-boot-offer --local-source ../magikos ../magikos-pkgs
./bin/magikos-iso-test release/<generated-iso>.iso --no-preview
```
