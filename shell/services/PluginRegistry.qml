...
QtObject {
  ...
  property string pluginsDir: home + "/.config/magikos/plugins"
  ...
  //   - the built-in bar option (`magikos.bar`) is active when `bar.id` is
  //     missing or set to `magikos.bar`.
  ...
  //     silently make `magikos launch bar-settings` a no-op. Turning one off
  ...
  function isEnabled(id) {
    ...
    if (manifest) {
      if (Array.isArray(manifest.kinds) && manifest.kinds.indexOf("bar") !== -1) {
        ...
        if (!selectedBar) selectedBar = "magikos.bar"
  ...
  function resolveEnabledId(id) {
    ...
    for (var candidate in installedPlugins) {
      ...
      var metadata = manifest && Util.isPlainObject(manifest.magikos) ? manifest.magikos : null
  ...
  // back, and so a plugin that is both a widget and a menu (magikos.menu)
  ...
  function barTarget(config, placement, fallbackSection) {
    ...
    var anchors = { left: "magikos.workspaces", center: "magikos.weather", right: "magikos.tray" }
  ...
  function activeCloneFor(config, sourceId) {
    for (var candidate in installedPlugins) {
      ...
      var candidateMetadata = candidateManifest && Util.isPlainObject(candidateManifest.magikos)
        ? candidateManifest.magikos : null
  ...
  function restoreCloneSource(config, cloneId, sourceId) {
    ...
    if (isBarOption) {
      if (sourceId === "magikos.bar") delete config.bar.id
    ...
    shellConfigMutator(function(config) {
      ...
      if (isBarOption) {
        if (value) {
          ...
          if (clonedFrom && clonedFrom !== "magikos.bar") config.bar.id = clonedFrom
  ...
  function parseScanOutput(text) {
    ...
    // `magikos.*` namespace is reserved for built-ins, including bar widgets
    // registered outside the manifest-based plugin registry.
    for (var tk in thirdParty) {
      if (firstParty[tk] || String(tk).indexOf("magikos.") === 0) {
        console.warn("PluginRegistry: plugin " + tk
          + " rejected: id is reserved for first-party Magikos plugins")
        continue
      }
  ...
  function rescan() {
    ...
    // Third-party plugins stay at the top level of ~/.config/magikos/plugins.
    var script = ""
