...
Item {
  ...
  required property string magikosPath
  ...
  property string magikosConfigDir: home + "/.config/magikos"
  ...
  Timer {
    ...
    onTriggered: {
      var svc = root.shell && root.shell.firstPartyServiceFor
        ? root.shell.firstPartyServiceFor("magikos.sway") : null
      if (svc) {
  ...
  // Sway-focused output name
  function focusedScreenName() {
    if (swayService) {
      return swayService.focusedOutputName
    }
    // Fallback: try to get from Quickshell screens
    var screens = Quickshell.screens
    for (var i = 0; i < screens.length; i++) {
      if (screens[i].focus) return screens[i].name
    }
    return ""
  }
  ...
  function customModuleSource(entry) {
    var source = BarModel.customModulePath(entry, home, magikosConfigDir)
    return source ? Util.fileUrl(source) : ""
  }

  Component.onCompleted: {
    // Try to find the Sway service from the shell
    if (root.shell && root.shell.firstPartyServiceFor) {
      swayService = root.shell.firstPartyServiceFor("magikos.sway")
    }
    applyBarConfig()
  }
  ...
  FileView {
    path: root.stateHome + "/magikos/current"
    watchChanges: true
    printErrors: false
  ...
  Process {
    id: barHiddenProbe
    running: true
    command: ["bash", "-c", "[[ -f $HOME/.local/state/magikos/toggles/bar-off ]] && echo yes || echo no"]
    stdout: SplitParser { onRead: function(line) { root.barHidden = String(line).trim() === "yes" } }
  }
  FileView {
    path: root.home + "/.local/state/magikos/toggles"
    watchChanges: true
    printErrors: false
  ...
  component BarPanel: PanelWindow {
    ...
    WlrLayershell.namespace: "magikos-bar"
  ...
  component DragGhostPanel: PanelWindow {
    ...
    WlrLayershell.namespace: "magikos-bar-drag-ghost"
  ...
  component BarMoveGhostPanel: PanelWindow {
    ...
    color: "transparent"
    exclusionMode: ExclusionMode.Ignore
    WlrLayershell.namespace: "magikos-bar-move-ghost"
    WlrLayershell.layer: WlrLayer.Overlay
    WlrLayershell.keyboardFocus: WlrKeyboardFocus.None
