...
BarWidget {
  ...
  // UseQApplication` - magikos's shell.qml does not, so every submenu click was
  ...
  function ownedByMagikos(item) {
    var layout = root.bar && root.bar.layoutConfig ? root.bar.layoutConfig : null
    return TrayModel.ownedByMagikos(item, layout)
  }
  ...
  function bucket(category) {
    ...
    for (var i = 0; i < values.length; i++) {
      ...
      if (ownedByMagikos(item)) continue
      if (category === "all") {
        result.push(item)
        continue
      }
  ...
  function persistTrayState(pinned, hidden) {
    ...
    var id = root.moduleName || "magikos.tray"
