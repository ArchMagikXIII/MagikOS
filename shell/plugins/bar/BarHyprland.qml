...
Item {
  ...
  // The magikos-shell host injects magikosPath from MAGIKOS_PATH.
  ...
  // to "monospace"), so changing the system font (via `magikos-font-set`)
  ...
  // Resolve the live bar-widget instance for a plugin id (e.g. "magikos.bluetooth").
  ...
  // and the flag is created/removed by `magikos-toggle-bar`.
  Process {
    id: barHiddenProbe
    running: true
