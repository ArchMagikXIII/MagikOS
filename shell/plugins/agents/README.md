...
`magikos-agent-usage-update` writes to `~/.local/state/magikos/agents/usage/`
...
`magikos plugin disable magikos.agents`.

## Data

Each agent is one JSON record in `~/.local/state/magikos/agents/usage/`,
written by `magikos-agent-usage-update`. That command runs one
`magikos-agent-usage-<agent>` collector per agent; the widget invokes it
...
configuration in `~/.config/magikos/agents/fireworks.json`:
...
- IPC: `magikos-shell magikos.agents <open|close|toggle|refresh|next>`.

## Settings

Settings live in the widget's entry in `~/.config/magikos/shell.json`. The
top-level keys can be set with
`magikos bar set magikos.agents <key> <value>`:
...
```bash
magikos bar set magikos.agents refreshIntervalSec 300 --json
magikos bar set magikos.agents syncDir '~/Sync/agent-usage'
```
...
```bash
magikos bar set magikos.agents providers '{
  "claude": { "enabled": true },
  "codex": { "enabled": false },
