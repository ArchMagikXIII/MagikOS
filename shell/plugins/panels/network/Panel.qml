...
Panel {
  id: root
  moduleName: "magikos.network"
  ipcTarget: "magikos.network"
  ...
  // `magikos-network-status --verbose` samples (~1.5s apart via detailsPoll).
  ...
  // Wi-Fi band state from `magikos-network-band`. `bandCurrent` is the band
  ...
  // The share card is its own panel plugin (magikos.wifiqr) so a replacement
  ...
  function summonWifiQr(forceDetect) {
    ...
    bar.shell.summon("magikos.wifiqr", JSON.stringify(payload))
  }
  ...
  function refresh(scanWifi) {
    ...
    if (!bandProc.running) {
      bandProc.command = ["magikos-network-band"]
  ...
  function setBand(band) {
    ...
    actionProc.command = ["magikos-network-band", band]
    actionProc.running = true
  }

  // The speed test is its own panel plugin (magikos.speedtest) so a
  ...
  function summonSpeedTest() {
    ...
    bar.shell.summon("magikos.speedtest", connection ? JSON.stringify({ connection: connection }) : "{}")
  }

  function dnsCommand(provider) {
    var command = "magikos-dns"
  ...
  function setDns(provider) {
    ...
    if (provider === "Custom") {
      var launcher = "magikos-launch-floating-terminal-with-presentation"
  ...
  Process {
    id: detailsProc
    command: ["magikos-network-status", "--verbose"]
