...
Item {
  ...
  property string historyPath: Quickshell.env("HOME") + "/.local/state/magikos/clipboard-history.json"
  property string captureScript: root.magikosPath + "/shell/plugins/clipboard/capture.sh"
  ...
  function applySelected(row) {
    ...
    if (row.entryType === "image") {
      Quickshell.execDetached([root.magikosPath + "/bin/magikos-clipboard-paste-file", row.mime, row.path])
    } else if (row.fullText) {
      Quickshell.execDetached([root.magikosPath + "/bin/magikos-clipboard-paste-text", "--shift-insert", "--history-index", String(row.historyIndex)])
    }
  }
  ...
  function copySelected(row) {
    ...
    root.opened = false
    if (row.entryType === "image") {
      Quickshell.execDetached([root.magikosPath + "/bin/magikos-clipboard-paste-file", "--copy-only", row.mime, row.path])
    } else if (row.fullText) {
      Quickshell.execDetached([root.magikosPath + "/bin/magikos-clipboard-paste-text", "--copy-only", "--history-index", String(row.historyIndex)])
    }
  }
  ...
  function openSelected(row) {
    if (!row) return
    root.opened = false
    Quickshell.execDetached([root.magikosPath + "/bin/magikos-clipboard-open", "--history-index", String(row.historyIndex)])
  }
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "magikos-clipboard"
