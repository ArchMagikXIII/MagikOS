...
//
// For Sway: corner radius and gaps are read from Sway config or use defaults.
...
QtObject {
  ...
  readonly property string menuFontFamily: {
    var override = Quickshell.env("MAGIKOS_MENU_FONT")
  ...
  function refresh() {
    // For Sway, we use default values or read from Sway config
    // Corner radius: default to 0 (square windows in Sway)
    // Gaps: default to 5 (common Sway gap setting)
    readSwayConfig.running = true
  }
  ...
  // Read Sway config to extract corner radius and gaps
  function applySwayConfig(raw) {
    try {
      var config = JSON.parse(raw || "{}")
      var configStr = config.config || ""

      // Try to extract corner radius from Sway config
      // Sway doesn't have corner radius by default, but some compositors add it
      // Default to 0 for standard Sway
      var radiusMatch = configStr.match(/corner_radius\s+(\d+)/)
      if (radiusMatch) {
        var n = parseInt(radiusMatch[1], 10)
        if (isFinite(n) && n >= 0) cornerRadius = n
      } else {
        cornerRadius = 0
      }

      // Try to extract gaps from Sway config
      var gapsMatch = configStr.match(/gaps\s+(?:inner|outer)\s+(\d+)/)
      if (gapsMatch) {
        var gapVal = parseInt(gapsMatch[1], 10)
        if (isFinite(gapVal) && gapVal >= 0) gapsOut = Math.max(0, Math.round(gapVal / 2))
      } else {
        gapsOut = 5
      }
    } catch (e) {
      // Sway not running or config parse failed — use defaults
      cornerRadius = 0
      gapsOut = 5
    }
  }
  ...
  function applyShellValues(values) {
    ...
    for (var fullKey in v) {
      ...
      if (section === "font") {
        if (key === "font-family") {
          root.fontFamily = String(raw)
        } else if (key === "base-size") {
          var ival = parseInt(raw, 10)
          if (isFinite(ival)) nextBase = ival
        } else {
          var nval = parseInt(raw, 10)
          if (isFinite(nval)) fontOut[key] = nval
        }
      } else if (section === "bar") {
  ...
  // Read Sway config for corner radius and gaps
  property Process readSwayConfig: Process {
    id: readSwayConfig
    command: ["swaymsg", "-t", "get_config"]
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: root.applySwayConfig(text)
    }
  }
  ...
  // Sway doesn't have a window-no-gaps toggle like Hyprland
  // We can watch a custom toggle file if needed
  property FileView windowNoGapsToggle: FileView {
    path: Quickshell.env("HOME") + "/.local/state/magikos/toggles/sway/window-no-gaps.conf"
    watchChanges: true
    printErrors: false
