...
QtObject {
  ...
  // component follows the fontconfig alias `magikos-font-set` writes.
  ...
  // alias path keeps working when the user runs `magikos font set`), but
  ...
  // MAGIKOS_MENU_FONT override for users who want a different family on the
  ...
  // Resolve the fontconfig alias to a concrete family name. `magikos font
  ...
  // `magikos toggle window-gaps` creates/removes this flag file. Hyprland
  ...
  property FileView windowNoGapsToggle: FileView {
    path: Quickshell.env("HOME") + "/.local/state/magikos/toggles/hypr/window-no-gaps.lua"
    watchChanges: true
    printErrors: false
